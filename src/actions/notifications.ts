...
export const sendNotification = async (userId: string, status: string, orderId: string) => {
  const tokenData = await getUserNotificationToken(userId);

  if (!tokenData.expo_notification_token) {
    return;
  }
  console.log("Expotoken", tokenData.expo_notification_token);
  await sendPushNotification({
    expoPushToken: tokenData.expo_notification_token,
    title: 'RN Shop - Order Status',
    body: `Your orderId: ${orderId} is now ${status}`,
  });
};